...
std::any Node::data( int column ) const
{
  using std::operator""s;

  switch ( column )
  {
  case 0: return parent == nullptr ? "Id"s : id;
  case 1:
  {
    if ( parent == nullptr ) return "Duration (μs)"s;
    return duration == 0 ? std::any{} : duration / 1000.0;
  }
  case 2: return ( parent == nullptr ) ? "Timestamp"s : util::isoDateMicros( timestamp );
  case 3: return ( parent == nullptr ) ? "File"s : file;
  case 4:
  {
    if ( parent == nullptr ) return "Line"s;
    return line == 0 ? std::any{} : line;
  }
  case 5: return ( parent == nullptr ) ? "Function"s : function;
  default: return {};
  }
}
