...
namespace spt::util
{
  ...
  std::variant<DateTime, std::string> parseISO8601( std::string_view date )
  {
    ...
    if ( date.size() < 10 )
    {
      apm::Application::instance()->log( "warn" ) << std::format( "Invalid date-time: {}", date );
    ...
    const auto parseZone = [&date]( int16_t millis ) -> std::variant<Tuple, std::string>
    {
      ...
      case '-':
      {
        ...
        if ( date[22] != ':' )
        {
          apm::Application::instance()->log( "warn" ) << std::format( "Invalid date-time: {}", date ) << " at 22 " << date[22] << " for size " << int( date.size() );
          return "Invalid datetime zone";
        }
      ...
      case '.':
        ...
        case '-':
          ...
          if ( date[idx + 3] != ':' )
          {
            apm::Application::instance()->log( "warn" ) << std::format( "Invalid date-time: {}", date ) << " at 26 " << date[26] << " for size " << int( date.size() );
            return "Invalid datetime zone";
          }
        ...
        case '-':
          ...
          if ( date[idx + 6] != ':' )
          {
            apm::Application::instance()->log( "warn" ) << std::format( "Invalid date-time: {}", date ) << " at 29 " << date[idx + 6] << " for size " << int( date.size() );
            return "Invalid datetime zone";
          }
