...
namespace date
{
  ...
  std::expected<DateTime, std::string> parseISO8601( std::string_view date )
  {
    ...
    if ( date.size() < 10 )
    {
      qWarning(DATE) << "Invalid date-time: " << date;
    ...
    const auto parseZone = [&date]( int16_t millis ) -> std::expected<Tuple, std::string>
    {
      ...
      case '-':
      {
        ...
        if ( date[22] != ':' )
        {
          qWarning(DATE) << "Invalid date-time: " << date << " at 22 " << date[22] << " for size " << int( date.size() );
          return O{ std::unexpect, "Invalid datetime zone" };
        }
      ...
      case '.':
        ...
        case '-':
          ...
          if ( date[idx + 3] != ':' )
          {
            qWarning(DATE) << "Invalid date-time: " << date << " at 26 " << date[26] << " for size " << int( date.size() );
            return O{ std::unexpect, "Invalid datetime zone" };
          }
        ...
        case '-':
          ...
          if ( date[idx + 6] != ':' )
          {
            qWarning(DATE) << "Invalid date-time: " << date << " at 29 " << date[idx + 6] << " for size " << int( date.size() );
            return O{ std::unexpect, "Invalid datetime zone" };
          }
